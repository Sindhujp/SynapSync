...
from google.appengine.api.urlfetch import DownloadError as DownloadError
...
OAUTH_APP_SETTINGS = {
    ...
    'twitter': {
        ...
        'consumer_key': 'AFQR11L6zE7JP8xbLACrA',
        'consumer_secret': 'oAgiZqLXDI2LP1eF9ntULiYGt76Q7VWzegrBkpJowEo',
	...
	'facebook': {
		...
		'default_api_prefix': 'http://facebook.com',
        'default_api_suffix': '.json',
...
def setTwitterAccounts(company):
    global twitterAccounts
...
class OAuthClient(object):
    ...
    def get_request_token(self):
        ...
        token = OAuthRequestToken(
            ...
            dbToken = temp[0].split('=')[1],
            dbTokenSecret = temp[0].split('=')[1]
            )
    ...
    def get_signed_body(self, __url, __token=None, __meth='GET',**extra_params):
        ...
        if __token is not None:
            kwargs['oauth_token'] = __token.dbToken
            key = self.service_key + encode(__token.dbTokenSecret)
        else:
...
class OAuthHandler(RequestHandler):
    ...
    def get(self, service, action=''):
        ...
        if url.startswith('http://localhost'):
            self.response.out.write('Cannot add social network account locally.')
            return
...
class MainHandler(RequestHandler):
    ...
    def get(self):
        ...
        write('Please authorize us by clicking <a href="/oauth/twitter/login" target="_blank">here</a>')
        write('<form method="post"><br>token: <input type="text" name="oauth_token" /><br>content: <input type="text" name="txtContent"/><br><input type="submit" name="btnPost" /></form>')
    ...
    def post(self):
        ...
        post = self.request.get('txtContent')
        #client.callback()
        self.response.out.write('token is :'+ token + '<br>' + 'post is :' + post + '<br>')
